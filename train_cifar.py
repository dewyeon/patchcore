...
class STPM(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.normal = hparams.cifar_normal
    ...
    def train_dataloader(self):
        image_datasets = CIFAR10_Dataset(root=os.path.join(args.dataset_path), transform=self.data_transforms, phase='train', normal=self.normal)
    ...
    def test_dataloader(self):
        test_datasets = CIFAR10_Dataset(root=os.path.join(args.dataset_path), transform=self.data_transforms, phase='test', normal=self.normal)
...
def get_args():
    ...
    parser.add_argument('--dataset_path', default='../data/cifar10/') # 'D:\Dataset\mvtec_anomaly_detection')#
    ...
    parser.add_argument('--project_root_path', default=r'./test/cifar') # 'D:\Project_Train_Results\mvtec_anomaly_detection\210624\test') #
    parser.add_argument('--save_src_code', default=True)
    parser.add_argument('--save_anomaly_map', default=True)
    parser.add_argument('--n_neighbors', type=int, default=9)
    parser.add_argument('--gpu', type=str, default='0')
    parser.add_argument('--cifar_normal', type=int, default=0)
...
if __name__ == '__main__':
    ...
    args = get_args()
    trainer = pl.Trainer.from_argparse_args(args, default_root_dir=os.path.join(args.project_root_path, str(args.cifar_normal)), max_epochs=args.num_epochs, gpus=args.gpu) #, check_val_every_n_epoch=args.val_freq,  num_sanity_val_steps=0) # ,fast_dev_run=True)
